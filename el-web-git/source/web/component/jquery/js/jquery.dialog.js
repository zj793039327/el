...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		//zhuweiwu add   原先是：that.title( config.title )
		that.title( config.title, config.id )
	...
	title: function( text, id )
	{
		...
		var DOM = this.DOM,
			...
			title = DOM.title[0],
			//zhuweiwu add
			title_bar = DOM.title_bar[0];
			
		if( text === false )
		{
			//zhuweiwu add   原先是：title.style.display = 'none';
			title_bar.style.display = 'none';
			title.innerHTML = '';
			//zhuweiwu add   原先是：border.addClass('ui_state_tips');
			id && id == 'Tips' && border.addClass('ui_state_tips');
		}
		else
		{
			//zhuweiwu add   原先是：title.style.display = '';
			title_bar.style.display = '';
